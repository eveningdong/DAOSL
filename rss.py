...
def policy_network(inputs,
  ...
  :return:  [1, 2]
  """
  with tf.variable_scope('policy', [inputs]) as sc:
    with slim.arg_scope(net_arg_scope(pool_pad='VALID',
      ...
      net = slim.fully_connected(net, args.num_groups, activation_fn=None)
  return net
...
def similarity_network(support_set, query_image):
  """
...
def nearest_neighbor_classifier(similarities, support_set_y):
  """
...
if __name__ == '__main__':
  img = tf.placeholder(tf.float32, [1, 28, 28, 1])
  reward = tf.placeholder(tf.float32, [1])
  probs = policy_network(img)
  print('probs', probs)

  policy = tf.multinomial(probs, 1)
  print('policy', policy)

  log_prob = tf.log(probs)
  print('log_prob', log_prob)
  act = tf.placeholder(tf.int32, [1])
